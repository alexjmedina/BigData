...
for dimension, values in dimension_data.items():
    dimension_df = pd.DataFrame({dimension: values}, index=project_names)
    dimension_df.index.name = "Project Name"
    dimension_df.to_csv(f"{dimension.replace(' ', '').lower()}.csv", index=True)
